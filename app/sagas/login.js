...
import {appStart, ROOT_COMPLETE_PROFILE, ROOT_INSIDE, ROOT_OUTSIDE, ROOT_VERIFY_EMAIL} from '../actions/app';
...
const handleLoginSuccess = function* handleLoginSuccess({ data }) {
	...
	}  else if (!data.salon_name.length){
		yield put(appStart({root: ROOT_COMPLETE_PROFILE}));
	} else {
