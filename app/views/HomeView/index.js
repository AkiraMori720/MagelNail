    ...
    init = async () => {
        ...
        this.unSubscribePost = postSubscribe.onSnapshot(async (querySnapShot) => {
            ...
            querySnapShot.forEach(doc => {
                const post = doc.data();
                if((!user.blocked || !user.blocked.includes(post.userId)) && user.followings.includes(post.userId) || user.userId === post.userId){
        ...
        return (
            ...
            >
                ...
                {((data.length > 0) || loading) ?
                    <FlatList
                        style={{flexGrow: 1, marginBottom: 80, marginTop: 50 + getStatusBarHeight(true)}}
                        data={data}
...
export default connect(mapStateToProps, mapDispatchToProps)(withActionSheet(withTheme(HomeView)));
